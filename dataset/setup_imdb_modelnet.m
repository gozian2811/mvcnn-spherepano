...
assert((nViews==12 && opts.useUprightAssumption) ...
    || (nViews==20 && opts.useUprightAssumption) ...
...
for ci = 1:length(imdb.meta.classes),
    ...
    valDir = fullfile(imdb.imageDir,imdb.meta.classes{ci},'val');
    testDir = fullfile(imdb.imageDir,imdb.meta.classes{ci},'test');
    trainOut = fullfile(strcat(imdb.imageDir,'v1'),imdb.meta.classes{ci},'train');
    valOut = fullfile(strcat(imdb.imageDir,'v1'),imdb.meta.classes{ci},'val');
    testOut = fullfile(strcat(imdb.imageDir,'v1'),imdb.meta.classes{ci},'test');
    if exist(trainDir,'dir'), hasTrain = true; else hasTrain = false; end;
    if exist(valDir,'dir'), hasVal = true; else hasVal = false; end;
    if exist(testDir,'dir'), hasTest = true; else hasTest = false; end;
    ...
    if hasTrain,
        ...
        for fi=1:length(files)
            ...
            image_outputnames{end+1} = fullfile(trainOut, files(fi).name);
        end
    end
    if hasVal,
        ...
        for fi=1:length(files)
            ...
            image_outputnames{end+1} = fullfile(valOut, files(fi).name);
        end        
    end
    if hasTest,
        if ~exist(testOut, 'dir'), mkdir(testOut); end
        files = dir(fullfile(testDir, ['*' opts.extmesh]));
        for fi=1:length(files)
            mesh_filenames{end+1} = fullfile(testDir, files(fi).name);
            image_outputnames{end+1} = fullfile(testOut, files(fi).name);
        end                
    end
    ...
    for fi=1:numShapes
        ...
        for ij=1:length(ims)
            imwrite( ims{ij}, sprintf('%s_%03d%s', image_outputnames{fi}(1:end-4), ij, opts.ext) );
        end
    end
    close(fig);
