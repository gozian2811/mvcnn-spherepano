...
if nViews>1, w = ceil(w/nViews) * nViews; end
npar = ceil([n1 n2]/w);
...
if nViews>1,
    parfor i=1:t,
        distcell{i} = multiview_alldist(x1cell{i},x2cell{i}, nViews);
        parfor_progress();
    end
else
    parfor i=1:t,
...
Dist = zeros(n1/nViews,n2/nViews,typeX1);
for i = 1:t,
    i1 = mod(i-1,npar(1))+1;
    i2 = floor((i-1)/npar(1))+1;
    if withSelf && i2>i1, 
    ...
    Dist((i1-1)*w/nViews+(1:sz1(i1)/nViews),(i2-1)*w/nViews+(1:sz2(i2)/nViews)) = dist_block;
    if opts.verbose, 
        if mod(i,10)==0, fprintf('.'); end
        if mod(i,200)==0, fprintf(' [%d/%d]\n',i,t); end;
    end
