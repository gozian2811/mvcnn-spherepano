...
for i=1:nShapes, 
  ...
  if ~isempty(opts.pad) && any(opts.pad>0), 
    ...
    %}
    imArrTmp = zeros(3*h, 3*w, size(imArr,3), size(imArr,4));
    for k=0:2,
        for l=0:2,
            imArrTmp((k*h+1):((k+1)*h), (l*w+1):((l+1)*w),:,:) = imArr;
        end
    end
    imArr = imArrTmp((h-opts.pad(1)+1):(2*h+opts.pad(2)), (w-opts.pad(3)+1):(2*w+opts.pad(4)),:,:);
  end
