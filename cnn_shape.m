...
for s=1:numel(opts.numEpochs), 
  ...
  if start >= 1
      ...
      load(fullfile(opts.expDir, sprintf('net-epoch-%d.mat',start)), 'net', 'info') ;
  ...
  [net,info] = trainFn(net, info, imdb, getBatchFn(opts, net.meta), ...
    'expDir', opts.expDir, ...
    net.meta.trainOpts, ...
    opts.train, ...
    'startEpoch', sum(opts.numEpochs(1:s-1)), ...
